...
import {d, get, randomId} from "./utils";
import {
    ACTION_ERROR,
    ACTION_SUCCESS,
    LOADING_END,
...
class Actions {
    ...
    confirm(action, payload) {
        return new Promise(resolve => {
            ...
            this.parent.events.once(ACTION_CONFIRM_RESOLVE, ([entry, result]) => {
                if (id === entry) {
                    resolve(result);
                }
            });

            this.parent.events.emit(ACTION_CONFIRM, [id, action, payload]);
        });
    };
    ...
    async _call(
        ...
        if (options.confirm) {
            const passed = await this.confirm(action, payload);
