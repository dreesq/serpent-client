...
class Actions {
    ...
    async setup(actions = {}) {
        ...
        if (!Object.keys(actions).length) {
            ...
            try {
                ...
                d('error', '(err) failed loading actions list', errors);
    ...
    finishTransaction(options, action, result, payload, start) {
        ...
        if (result.errors) {
            ...
            if (action === null) {
                for (const key in Object.keys(result.data)) {
                    this.parent.events.emit(ACTION_ERROR, [key, result.data[key], payload[key]]);
                }
            } else {
        ...
        const end = +new Date();
        d('info', `(ok) (${action}) in (${end - start} ms)`, result);
        return result;
    }
