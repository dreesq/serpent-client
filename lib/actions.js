        ...
        try {
            ...
            if (withProgress) {
                config.onUploadProgress = e => {
                    ...
                    this.parent.events.emit(ACTION_PROGRESS, [names, percent]);
                };
            ...
            const {data: actionResults} = await this.parent.http.post(Config.get('handler'), form, config);
    ...
    async _call(action, data = {}) {
        ...
        try {
            ...
            if (typeof FormData !== 'undefined' && payload instanceof FormData && withProgress) {
                config.onUploadProgress = e => {
                    let percent = Math.floor((e.loaded * 100) / e.total);
                    this.parent.events.emit(ACTION_PROGRESS, [action, percent]);
                };
